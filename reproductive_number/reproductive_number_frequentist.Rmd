...
The first plot shows the relationship between worker number and male number. This is roughly linear. Because this is count data a poisson or NB model will model log(male number) ~ worker number, shown in plot 2. This isn't as linear anymore. By modelling log(male number) ~ log(worker number), we achieve a better linear relationship in the poisson/nb model, which when expressed as male number ~ e^log(worker_number) has more of an exponential shape. 
